...
export function configurePkgJson(basePkgJson: BasePkgJson, cjs: boolean, configs: ConfigMap) {
  ...
  if (cjs) {
    ...
    newPkgJson.exports.require = './dist/index.cjs';
  }
...
export function readConfigs(basePkgJson: BasePkgJson, cjs: boolean, configs: ConfigMap) {
  console.info('Reading configs');
  for (const config of basePkgJson.files) {
    if (!(config === 'dist') && (!cjs && !(config === 'tsconfig.cjs.json'))) {
      try {
        const file = fs.readFileSync(`${tsConfigPath}/${config}`, encoding).toString();
